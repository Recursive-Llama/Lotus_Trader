#!/usr/bin/env python3
"""
Test Ethereum Uniswap Client (v2/v3 fallback)
"""
...
class TestEvmUniswapClientEth:
    """Test Ethereum Uniswap client functionality"""
    
    @pytest.fixture
    def client(self):
        """Create client instance"""
        with patch.dict(os.environ, {'ETH_RPC_URL': 'https://test.eth.rpc'}):
            with patch('web3.Web3') as mock_web3_class:
                mock_web3 = Mock()
                mock_web3.is_connected.return_value = True
                mock_web3_class.return_value = mock_web3
                return EthUniswapClient()
    ...
    def test_eip1559_transaction_building(self, client, mock_web3):
        """Test EIP-1559 transaction building"""
        with patch.object(client, 'w3', mock_web3):
            # Mock fee history
            mock_web3.eth.fee_history.return_value = {
                'baseFeePerGas': [20000000000, 21000000000],
            ...
            tx = client._build_eip1559_transaction(
    ...
    @pytest.mark.skipif(not os.getenv('ETH_RPC_URL'), reason="Requires ETH_RPC_URL for live test")
    def test_live_v2_liquidity_check(self, client):
        """Test live v2 liquidity check (requires RPC)"""
        # Test with USDC on Ethereum (should have v2 liquidity)
        usdc_address = "0xA0b86a33E6441b8C4C8C0C4C0C4C0C4C0C4C0C4C"  # USDC on ETH
        amounts = client.v2_get_amounts_out(
            client.weth_address,
            usdc_address,
