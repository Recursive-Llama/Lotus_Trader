-- Memory Strands Database Schema
-- Enhanced ad_strands table with module-specific resonance scoring and preserved clustering infrastructure
-- SINGLE STRANDS TABLE: All strand types (social_lowcap, decision_lowcap, pm_action, etc.) use this table with different `kind` values
...
CREATE TABLE ad_strands (
    ...
    kind TEXT,                               -- 'pattern'|'prediction_review'|'conditional_trading_plan'|'trading_decision'|'execution_outcome'|'braid'|'meta_braid'|'meta2_braid'|'social_lowcap'|'decision_lowcap'|'pm_action'
...
-- Function to calculate resonance scores (learning_queue removed - strands processed directly)
...
COMMENT ON TABLE ad_strands IS 'SINGLE STRANDS TABLE: All strand types use this table with different kind values (social_lowcap, decision_lowcap, pm_action, pattern, etc.)';
COMMENT ON COLUMN ad_strands.kind IS 'Strand type: social_lowcap, decision_lowcap, pm_action, pattern, prediction_review, etc.';
