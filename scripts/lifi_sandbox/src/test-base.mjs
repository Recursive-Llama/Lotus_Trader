...
const basePrivateKey = normalizePrivateKey(maybeEnv('BASE_WALLET_PRIVATE_KEY'))
...
const main = async () => {
  ...
  console.log(`RPC: ${baseRpcUrl.replace(/\/\/.*@/, '//***@')}`)

  const fromToken = await resolveToken(ChainId.BAS, ['USDC', 'USDBC'])
  const toToken = await resolveToken(ChainId.BAS, ['WETH'])
