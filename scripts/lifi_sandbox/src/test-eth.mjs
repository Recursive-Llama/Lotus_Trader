...
const ethPrivateKey = normalizePrivateKey(maybeEnv(['ETH_WALLET_PRIVATE_KEY', 'ETHEREUM_WALLET_PRIVATE_KEY']))
const ethRpcUrl = maybeEnv(['ETH_RPC_URL', 'ETHEREUM_RPC_URL'])

if (!ethPrivateKey || !ethRpcUrl) {
  console.error('ETH_WALLET_PRIVATE_KEY (or ETHEREUM_WALLET_PRIVATE_KEY) and ETH_RPC_URL (or ETHEREUM_RPC_URL) are required')
  process.exit(1)
}
...
const walletClient = createWalletClient({
  account,
  chain: mainnet,
  transport: http(ethRpcUrl, { retryCount: 2 }),
})
...
const main = async () => {
  console.log('\n=== Ethereum Spot Swap Test ===\n')
  console.log(`Integrator: ${integrator}`)
  console.log(`Dry run: ${dryRun}`)
  console.log(`RPC: ${ethRpcUrl.replace(/\/\/.*@/, '//***@')}`)

  const fromToken = await resolveToken(ChainId.ETH, ['USDC', 'USDC.E'])
  const toToken = await resolveToken(ChainId.ETH, ['WETH'])
