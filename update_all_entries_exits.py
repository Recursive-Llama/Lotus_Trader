...
async def update_all_entries_exits():
    supabase = SupabaseManager()
    
    print('=== UPDATING ALL ENTRIES, EXITS, AND EXIT_RULES ===')
    print()
    ...
    for ticker, pos_data in positions_to_update.items():
        print(f'--- {ticker} ---')
        ...
        if result.data:
            ...
            print(f'  Adding entry #{next_entry_number} for {additional_tokens:.2f} tokens')
            print(f'  Updating {len(updated_exits)} exit prices based on {new_avg_price:.8f} avg price')
    ...
    print(f'=== SUMMARY ===')
    print(f'Total positions updated: {updates_made}/{len(positions_to_update)}')

if __name__ == "__main__":
    asyncio.run(update_all_entries_exits())
