#!/usr/bin/env python3
"""
Workflow script to run full backtest process for a token:
1. Backfill 1h data
...
from backtester.backtest_uptrend_v2 import main as backtest_main
...
def run_backtest(ticker: str, days: int) -> None:
    ...
    sys.argv = ["backtest_uptrend_v2.py", ticker, str(days)]
    try:
        backtest_main()
    except SystemExit:
        pass
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"Backtest Workflow for {ticker} ({days} days)")
    print(f"{'='*60}\n")
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"✅ Complete! Check backtest_results_{ticker}_*.png for chart")
        print(f"{'='*60}\n")
