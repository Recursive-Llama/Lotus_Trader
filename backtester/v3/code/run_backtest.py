#!/usr/bin/env python3
"""
Workflow script to run full backtest process for a token (V3):
1. Backfill 1h data
...
# File is at: backtester/v3/code/run_backtest.py
...
backtest_path = os.path.join(os.path.dirname(__file__), "backtest_uptrend_v3.py")
spec = importlib.util.spec_from_file_location("backtest_uptrend_v3", backtest_path)
...
def run_backtest(ticker: str, days: int) -> None:
    ...
    sys.argv = ["backtest_uptrend_v3.py", ticker, str(days)]
    try:
        backtest_main()
    except SystemExit:
        pass
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"Backtest Workflow V3 for {ticker} ({days} days)")
    print(f"{'='*60}\n")
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"✅ Complete! Check backtester/v3/backests/backtest_results_{ticker}_*.png for chart")
        print(f"{'='*60}\n")
