# Archived: PM Learning Lever Map (2024 draft)

*This document captures the original lever map proposal prior to the v5.3 learning overhaul. It is kept here for reference; see `PM_Learning_System.md` for the current architecture.*
...
**DM variant**: If a lesson lives entirely in DM dimension space (curator/chain/cap/intent/etc.), we emit `effect.alloc_multiplier` (bounded 0.7–1.3) and apply it at the end of the DM coefficient pipeline.
